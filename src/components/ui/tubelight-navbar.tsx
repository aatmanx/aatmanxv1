...
export function NavBar({ items, className }: NavBarProps) {
  ...
  return (
    <div
      className={cn(
        "fixed bottom-0 sm:top-0 left-1/2 -translate-x-1/2 z-50 mb-6 sm:mb-0 sm:mt-6 sm:pt-0",
        className,
      )}
    >
      <div className="flex items-center gap-1 sm:gap-2 bg-card/60 border border-border backdrop-blur-lg py-1 px-1 rounded-full shadow-lg">
        {items.map((item) => {
