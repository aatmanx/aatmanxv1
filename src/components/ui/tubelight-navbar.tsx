...
export function NavBar({ items, className, trailing }: NavBarProps) {
  ...
  return (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 select-none">
      <div className={cn("flex items-center gap-1 sm:gap-2 bg-card/70 border border-border backdrop-blur-lg py-1 px-1 rounded-full shadow-lg", className)}>
        {items.map((item) => {
