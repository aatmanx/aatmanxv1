-- Real Estate Questionnaire System v2
-- Rebuilds questionnaire storage from scratch for AI website generation pipeline

-- ---------------------------------------------------------------------------
-- Drop legacy questionnaire tables
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- questionnaires — master record per session / user
-- ---------------------------------------------------------------------------
CREATE TABLE public.questionnaires (
  ...
  ai_generation_status JSONB NOT NULL DEFAULT '{
    "homepage_content": "pending",
    "about_page": "pending",
    "property_descriptions": "pending",
    "seo_metadata": "pending",
    "faqs": "pending",
    "blog_content": "pending"
  }'::jsonb,
...
CREATE INDEX questionnaires_template_category_idx ON public.questionnaires (template_category);
...
-- ---------------------------------------------------------------------------
-- questionnaire_answers — per-question immediate persistence
-- ---------------------------------------------------------------------------
...
CREATE POLICY "Users can manage own questionnaire answers"
...
-- ---------------------------------------------------------------------------
-- questionnaire_files — uploaded asset references
-- ---------------------------------------------------------------------------
...
CREATE POLICY "Users can view own questionnaire files"
  ON public.questionnaire_files FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.questionnaires q
      WHERE q.id = questionnaire_id AND q.user_id = auth.uid()
...
CREATE POLICY "Users can manage own questionnaire files"
  ON public.questionnaire_files FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.questionnaires q
      WHERE q.id = questionnaire_id AND q.user_id = auth.uid()
...
-- ---------------------------------------------------------------------------
-- Link websites to questionnaires
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Storage policies for anonymous session uploads (authenticated only for now)
-- Bucket questionnaire-assets already exists from prior migration
-- ---------------------------------------------------------------------------
